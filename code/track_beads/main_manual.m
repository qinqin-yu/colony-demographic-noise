...
for i = 1:length(inds)
    ind = inds(i);
    strain_name = num2str(T.strains_all(ind));
    masscut = T.masscut_all(ind);
    maxd = T.maxd_all(ind);
    radius = T.radius_all(ind);
    barrg = T.barrg_all(ind);
    xmin = T.xmin_all(ind)/conv;
    ...
    if isfile(tracking_par_filename)
        ...
        T = table(experiment_all, strains_all, radius_all, masscut_all, maxd_all, barrg_all, xmin_all, xmax_all, ymin_all, ymax_all);
    end

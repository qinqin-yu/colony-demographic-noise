...
%if a particle gets lost more than memory times, then it gets dumped out
%if a lost particle has fewer than goodenough points in its trajectory, then it
%gets dumped
...
for i=istart+1:z %always starts at 2 (while inipos not implemented) 
    ...
    if m>0  
        ...
        if notnsqrd
            ...
            ntrack=length( wh );
            if ntrack == 0
                warning('Warning - No valid particles to track!')
            end
            xmat=ones(ntrack,m);
            for ng=1:m
                xmat(:,ng)=ng;
